 ...
 * RawJavaParser 单元测试
 * <p>
 * 用例设计与 {@link JavaParserTest} 保持一致，确保 RawJavaParser 与 JavaParser 行为等价。
 */
class RawJavaParserTest {

    private RawJavaParser parser;
    private RawJavaParser timeoutParser;

    private static Map<String, ConfigItem<?>> buildConfigMap() {
        Map<String, ConfigItem<?>> map = new HashMap<>();
        RawLanguageParser.registerCommonConfigs(map);
        return map;
    }
    ...
    private static Map<String, ConfigItem<?>> buildZeroTimeoutConfigMap() {
        Map<String, ConfigItem<?>> map = new HashMap<>();
        RawLanguageParser.registerCommonConfigs(map);
        map.put(RawLanguageParser.CONFIG_KEY_TIMEOUT, ConfigItems.LONG(0L).build());
        return map;
    }

    @BeforeEach
    void setUp() {
        parser = new RawJavaParser(buildConfigMap());
        timeoutParser = new RawJavaParser(buildZeroTimeoutConfigMap());
    }
    ...
    // ==================== RawJavaParser 特有：registerConfigs ====================

    @Test
    @DisplayName("registerConfigs 应注册三个通用配置项")
    void testRegisterConfigs() {
        RawJavaParser p = new RawJavaParser();
        Map<String, ConfigItem<?>> configMap = new HashMap<>();
        p.registerConfigs(configMap);

        assertTrue(configMap.containsKey(RawLanguageParser.CONFIG_KEY_TIMEOUT));
        assertTrue(configMap.containsKey(RawLanguageParser.CONFIG_KEY_MAX_PARSE_SIZE));
        assertTrue(configMap.containsKey(RawLanguageParser.CONFIG_KEY_MAX_REGEX_SIZE));
